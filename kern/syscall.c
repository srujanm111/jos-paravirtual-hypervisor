...
// srcva should be assumed to be already converted to a host virtual address (in
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (curenv->env_type == ENV_TYPE_GUEST && target_env->env_ipc_dstva < (void*) UTOP) {
		// guest -> host
		pp = pa2page(PADDR(srcva));
		r = page_insert(target_env->env_pml4e, pp, target_env->env_ipc_dstva, perm);
		if (r < 0) {
			cprintf("[%08x] page_insert %08x failed in sys_ipc_try_send (%e)\n", curenv->env_id, srcva, r);
			return r;
		}
		target_env->env_ipc_perm = perm;
	} else if (target_env->env_type == ENV_TYPE_GUEST && srcva < (void*) UTOP) {
		// host -> guest
		if (PGOFF(srcva) != 0) {
			cprintf("[%08x] srcva %08x not aligned in sys_ipc_try_send\n", curenv->env_id, srcva);
			return -E_INVAL;
		}
		if ((~perm & (PTE_U|PTE_P)) || (perm & ~PTE_SYSCALL)) {
		...
		#ifndef VMM_GUEST
		r = ept_page_insert(target_env->env_pml4e, pp, target_env->env_ipc_dstva, perm);
		#endif
		if (r < 0) {
		...
		target_env->env_ipc_perm = perm;
	} else if (srcva < (void*) UTOP && target_env->env_ipc_dstva < (void*) UTOP) {
		// host -> host
		if (PGOFF(srcva) != 0) {
			cprintf("[%08x] srcva %08x not aligned in sys_ipc_try_send\n", curenv->env_id, srcva);
			return -E_INVAL;
    ...
    target_env->env_tf.tf_regs.reg_rax = 0;
    target_env->env_status = ENV_RUNNABLE;

	if (target_env->env_type == ENV_TYPE_GUEST) {
		target_env->env_tf.tf_regs.reg_rsi = value;
	}
...
static int
sys_ept_map(envid_t srcenvid, void *srcva, envid_t guestid, void* guest_pa, int perm) {
	int r1, r2;
	struct Env *srcenv;
	struct Env *guestenv;
	r1 = envid2env(srcenvid, &srcenv, 1);
	r2 = envid2env(guestid, &guestenv, 1);

	// srcenvid and/or guest doesn't currently exist,
	// or the caller doesn't have permission to change one of them
	if (r1 < 0 || r2 < 0) 
		return -E_BAD_ENV;

	if ((uint64_t) srcva >= UTOP 
		 || (uint64_t) guest_pa >= guestenv->env_vmxinfo.phys_sz 
		 || PGOFF(srcva) != 0 
		 || PGOFF(guest_pa) != 0)
		return -E_INVAL;

	pml4e_t *pml4e = srcenv->env_pml4e;
	pte_t *ppte;
	struct PageInfo *page_info = page_lookup(pml4e, srcva, &ppte);
	...
	void *hva = page2kva(page_info);
	epte_t *eptrt = guestenv->env_pml4e;
	int overwrite = 0;
	r1 = ept_map_hva2gpa(eptrt, hva, guest_pa, perm, overwrite);
	
	// if there's no memory to allocate any necessary page tables
	if (r1 < 0)
		return r1;
...
static envid_t
	...
	if ( !vmx_check_ept() ) {
		cprintf("Processor does not support EPT\n");
		return -E_NO_EPT;
	} else {
