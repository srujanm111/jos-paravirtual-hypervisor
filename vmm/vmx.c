...
bool vmx_check_support() {
	...
	uint32_t vmx_support = BIT(ecx, 5);
	if (vmx_support == 1) {
		return true;
	} else {
 ...
 */
bool vmx_check_ept() {
	// we need to first check if secondary VMX controls are enabled
	// by checking primary VMX controls in IA32_VMX_PROCBASED_CTLS
	uint64_t primary_msr = read_msr(IA32_VMX_PROCBASED_CTLS);
	uint64_t secondary_enabled = BIT(primary_msr, 63);
	if (secondary_enabled == 1) {
		// check if EPT is enabled in secondary VMX controls MSR
		uint64_t secondary_msr = read_msr(IA32_VMX_PROCBASED_CTLS2);
		uint64_t ept_enabled = BIT(secondary_msr, 33);
		if (ept_enabled == 1) {
			return true;
		}
	}
...
void vmexit() {
	...
	int exit_reason = vmcs_read32(VMCS_32BIT_VMEXIT_REASON); 
...
void asm_vmrun(struct Trapframe *tf) {
	...
	// HINT LAB 1: tf_ds should be set to the number of times the current env has run 
	...
	asm(
	        ...
	        /* Loads the guest general purpose registers from the trap frame without clobbering flags
	         *
	         */
	        "mov %c[rax](%0), %%rax \n\t"
	        "mov %c[rbx](%0), %%rbx \n\t"
	         ...
	         * that you don't do any comparison that would clobber the condition code, set
	         * above.
	         */
			"je .Llaunch \n\t"
			"vmresume \n\t"
	        ...
	        "pop %0 \n\t"
	        /* Saves the general purpose guest registers and cr2 back to the trapframe.
	         *
...
int vmx_vmrun( struct Env *e ) {
	...
	// HINT LAB 1: The following if statement should be true when the environment has only run once.
	if (e->env_runs == 1) {
