...
static int 
ept_lookup_gpa(epte_t* eptrt, void *gpa, int create, epte_t **epte_out) {
	if (eptrt == NULL)
		return -E_INVAL;
	...
	for (i = EPT_LEVELS - 1; i > 0; --i) {
		...
		if (!epte_present(dir[idx])) {
			// if create == 0 and the intermediate page table entries are missing
			if (create == 0)
				return -E_NO_ENT;
			...
			dir[idx] = (addr & EPTE_ADDR) | __EPTE_FULL;
		}
		dir = (epte_t *) epte_page_vaddr(dir[idx]);
	}

	if (epte_out != NULL)
		*epte_out = (epte_t *) &dir[ADDR_TO_IDX(gpa, 0)];
...
int ept_page_insert(epte_t* eptrt, struct PageInfo* pp, void* gpa, int perm) {
	epte_t *epte_out;
	int r = ept_lookup_gpa(eptrt, gpa, 1, &epte_out);

	if (r < 0)
		return r;

	if (epte_present(*epte_out)) {
		struct PageInfo *old_page = pa2page(epte_addr(*epte_out));
		page_decref(old_page);
	}
	
	*epte_out = ((uint64_t) page2pa(pp) & EPTE_ADDR) | perm;
	pp->pp_ref++;
...
int 
ept_map_hva2gpa(epte_t* eptrt, void* hva, void* gpa, int perm, int overwrite) {
	int r;
	int create = 1;
	epte_t *epte_out;
	r = ept_lookup_gpa(eptrt, gpa, create, &epte_out);

	if (r < 0)
		return r;

	// the mapping already exists and overwrite is set to 0
	if (epte_present(*epte_out) && overwrite == 0)
		return -E_INVAL;
	
	*epte_out = (PADDR(hva) & EPTE_ADDR) | perm;

	// set the type to EPTE_TYPE_WB and set __EPTE_IPAT flag
	*epte_out |= (__EPTE_TYPE(EPTE_TYPE_WB) | __EPTE_IPAT);
